...
public:
  WDBCTableReader(std::shared_ptr<Stream::StreamReader> streamReader,
  ...
  std::vector<Structures::BlizzardDatabaseRowDefiniton>
  RecordDefinition() override;
...
public:
  StringTable() { _stringTable = std::map<std::string, int>(); }
...
template <typename T>
inline auto write(std::ostream &stream, T const &val) -> void {
...
public:
  WDBCTableWriter(std::ofstream &stream,
                  Structures::VersionDefinition &versionDefinition)
      : _stream(stream), _versionDefinition(versionDefinition) {}
  ~WDBCTableWriter() {}
  ...
  bool Write(const std::vector<Structures::BlizzardDatabaseRow> &rows) {
    ...
    for (auto const &column :
         _versionDefinition.versionDefinitions.definitions) {
    ...
    for (auto const &row : rows) {
      ...
      for (auto const &column :
           _versionDefinition.versionDefinitions.definitions) {
        auto columnDefintion =
            _versionDefinition.columnDefinitions[column.name];
    ...
    _stream.write(reinterpret_cast<const char *>(stream.getBuf()),
                  stream.getLength());
    _stream.write(reinterpret_cast<const char *>(stringTableBuffer.data()),
