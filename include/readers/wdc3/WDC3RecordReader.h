...
public:
  WDC3RecordReader(std::shared_ptr<Stream::StreamReader> streamReader,
  ...
  ReadRecord(int indexOfId, Structures::WDC3Section &section,
             Stream::BitReader &reader,
             std::vector<Structures::FieldMeta> &fieldMetaData,
             std::vector<Structures::ColumnMetaData> &columnMetaData,
             std::map<int, std::vector<Structures::Int32>> &palletMetaData,
             std::map<int, std::map<int, Structures::Int32>> &commonMetaData,
             Structures::ReferenceData &referenceData,
             std::vector<int> &indexData);

private:
  template <typename T>
  T GetFieldValue(int Id, Stream::BitReader &reader,
                  Structures::FieldMeta &fieldMeta,
                  Structures::ColumnMetaData &columnMeta,
                  std::vector<Structures::Int32> &palletData,
                  std::map<int, Structures::Int32> &commonData) {
    ...
    case Structures::CompressionType::SignedImmediate: {
      return reader
          .ReadSignedValue64(columnMeta.compressionData.Immediate.BitWidth)
          .As<T>();
    }
    case Structures::CompressionType::Immediate: {
      return reader.ReadValue64(columnMeta.compressionData.Immediate.BitWidth)
          .As<T>();
    }
    ...
    case Structures::CompressionType::Pallet: {
      auto value =
          reader.ReadUint32(columnMeta.compressionData.Pallet.BitWidth);
  ...
  template <typename T>
  std::vector<T>
  GetFieldArrayValue(int Id, Stream::BitReader &reader,
                     Structures::FieldMeta &fieldMeta,
                     Structures::ColumnMetaData &columnMeta,
                     std::vector<Structures::Int32> &palletData,
                     std::map<int, Structures::Int32> &commonData) {
  ...
  std::vector<std::string> GetFieldStringArrayValue(
      int offset, int startOfStringTable, Stream::BitReader &reader,
      Structures::FieldMeta &fieldMeta, Structures::ColumnMetaData &columnMeta,
      std::vector<Structures::Int32> &palletData,
      std::map<int, Structures::Int32> &commonData) {
    ...
    case Structures::CompressionType::None: {
      ...
      auto entires = columnMeta.Size / bitSize;
      for (auto i = 0; i < entires; i++) {
        auto entryIndex = (reader.Position >> 3) + offset +
                          reader.ReadValue64(bitSize).As<int>();
